...
insert  into `transaction_details2`(`transdetailsId`,`amountTendered`,`Discount`,`delfee`,`totaDueAmount`,`currentBal`,`tDate`,`transNo`,`clientid`,`numberofitems`,`isPaid`) values (1,51000.00,5,500.00,62320.00,11820.00,'11-22-2022','202211220001',202206160005,4,0);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_paymentprint50` AS select `td2`.`transNo` AS `transno`,`td2`.`clientid` AS `clientid`,`pc`.`paymenttype` AS `paymenttype`,`pt`.`paymentname` AS `paymentname`,`td2`.`Discount` AS `discount`,`td2`.`delfee` AS `delfee`,`td2`.`totaDueAmount` AS `totadueamount`,`pc`.`paymentdetails` AS `paymentdetails`,`pc`.`Amount` AS `amount`,`td2`.`numberofitems` AS `numberofitems`,`td2`.`currentBal` AS `currentBal`,`td2`.`tDate` AS `tDate` from ((`transaction_details2` `td2` join `payment_copy` `pc` on(`pc`.`transno` = `td2`.`transNo`)) join `paymenttype` `pt` on(`pt`.`paymentid` = `pc`.`paymenttype`)) */;
